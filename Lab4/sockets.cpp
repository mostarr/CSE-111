...
void base_socket::close()
{
  ...
  if (status < 0)
    throw socket_sys_error("close(" + to_string(socket_fd) + ")");
  socket_fd = CLOSED_FD;
}
...
void base_socket::create()
{
  ...
  if (socket_fd < 0)
    throw socket_sys_error("socket");
  int on = 1;
  int status = ::setsockopt(socket_fd, SOL_SOCKET, SO_REUSEADDR,
                            &on, sizeof on);
  if (status < 0)
    throw socket_sys_error("setsockopt");
}
...
void base_socket::bind(const in_port_t port)
{
  ...
  int status = ::bind(socket_fd,
                      ...
                      sizeof socket_addr);
  if (status < 0)
    throw socket_sys_error("bind(" + to_string(port) + ")");
}
...
void base_socket::connect(const string host, const in_port_t port)
{
  ...
  if (hostp == NULL)
    throw socket_h_error("gethostbyname(" + host + ")");
  socket_addr.sin_family = AF_INET;
  socket_addr.sin_port = htons(port);
...
string to_string(const in_addr &ipv4_addr)
{
  ...
  if (result == NULL)
    throw socket_sys_error("inet_ntop");
  return result;
}
...
{
  if (host == nullptr)
    throw socket_h_error("gethostbyname");
  return host->h_name;
}
...
hostinfo::hostinfo(hostent *host) : hostname(init_hostname(host)),
...
string localhost()
{
  ...
  if (rc < 0)
    throw socket_sys_error("gethostname");
  return hostname;
}
